...
public class SpeedGenerator {
	 ...
	 */
	public SpeedGenerator(double minSpeed, double maxSpeed, double startAccel, double stopAccel, double radius1,
			double radius2, double lookahead) {
	...
	public Function<Double, Double> getSpeedFunction(Path path) {
		...
		return t -> {
			...
			final double curvature = segment.getCurvature().apply(
					(lookaheadt * length - path.getTotalOfCompletedPaths(lookaheadt * length)) / segment.getLength());
