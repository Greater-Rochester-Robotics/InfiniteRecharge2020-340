...
public class PathSegment {
	...
	public PathSegment(Vec2 start, Vec2 mid1, Vec2 mid2, Vec2 end, double length) {
		...
		dx = t -> {
			return (double) (3 * (end.x + 3 * mid1.x - 3 * mid2.x - start.x) * Math.pow(t, 2)
					- 6 * (2 * mid1.x - mid2.x - start.x) * t + 3 * mid1.x - 3 * start.x);
		};
		dy = t -> {
			return (double) (3 * (end.y + 3 * mid1.y - 3 * mid2.y - start.y) * Math.pow(t, 2)
					- 6 * (2 * mid1.y - mid2.y - start.y) * t + 3 * mid1.y - 3 * start.y);
		};
